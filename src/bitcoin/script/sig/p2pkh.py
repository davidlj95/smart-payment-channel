...
class P2PKH(ScriptSig):
    ...
    def sign(self, key, script=None, hashtype=DEFAULT_HASHTYPE):
        ...
        assert self.parent is not None and self.parent.tx is not None, \
            "To sign the P2PKH scriptSig, there must be an input related " + \
            "to the scriptSig that is related to a transaction"
        assert isinstance(self.parent.tx, SignableTx), "To sign the P2PKH " + \
        ...
        # Sign and add
        self._signature = self._parent.tx.sign(
            key, self._parent, script, hashtype)
